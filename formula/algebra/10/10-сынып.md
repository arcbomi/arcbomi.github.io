...
$$
\begin{align*}
&\text{Графикке жанама}:y=f^\prime(x_0)(x-x_0)+f(x_0) \\
&\text{Егер } f^\prime(x_0)=0 \text{ болса, онда функция } x_0 \text{ нүктесінде экстремум мәнін(минимум немесе максимум) қабылдайды} \\
\end{align*}
$$
$$
\begin{align*}
&1)\text{Егер } f(x) \text{ функциясы [a,b] интервалында дифференциацияланатын функция болса, және } f(a)=f(b) \text{ болса онда } \exists x_0:f^\prime(x_0)=0 (x_0\in [a,b])\  \textbf{(Ролль)}\\
&2)\text{Егер } f(x) \text{ функциясы [a,b] интервалында дифференциацияланатын функция болса, онда } \exists x_0\in [a,b]:f^\prime(x_0)=\frac{f(a)-f(b)}{a-b} \ \textbf{(Лагранж)} \\
&3)f(x), g(x)\text{ функциялары [a,b] интервалында дифференфиацияланатын функциялар болсын және  } g^\prime(x)\neq 0 \ \forall x\in [a,b]; \text{онда } \exists x_0\in[a,b]: \frac{f(b)-f(a)}{g(b)-g(a)}=\frac{f^\prime(x_0)}{g^\prime(x_0)} \ \textbf{(Коши)}\\
&\text{Егер біз үшінші тұжырымда f(a)=g(a)=0 деп алып, кейбір манипуляциялар(логикалық) қолдансақ, физмат жек көретін Лопиталь ережесін аламыз.}
\end{align*}
$$
# Additional
$$
\begin{align*}
&\text{Егер } F^\prime(x)=f(x) \text{ болса, онда }F(x) \text{ функциясы } f(x) \text{ функциясының алғашқы функциясы деп аталады, олар }  F(x)=\int f(x) \text{ деп белгіленеді}\\
&\text{Егер константа туындысының 0-ге тең екенін еске түсірсек, онда } F(x) \text{ функцияларының шексіз көп екенін түсінуге болады} \\
&\text{Әдетте біз оларды } F(x)=\int f(x) + C \text{ деп белгілейміз, оны } \textbf{функцияның интегралы деп атаймыз.} \\
&\textbf{Оларды ажырата білу керек: мысалы, } f(x)=x \textbf{ үшін } g(x)=\frac{x^2}{2} \textbf{ функциясы оның алғашқы функциясы деп аталады, ал } h(x)=\frac{x^2}{2}+C\ (C\in R, C-const)  \textbf{ функциясы оның интегралы деп аталады.} \\
&\textbf{Интегралдар туралы көбірек мәлімет 11-сыныпта болады.} \\
\end{align*}
$$
$$
\begin{align*}
&\text{Егер } f(x) \text{функциясының екінші туындысы анықталған болса, онда келесі теңсіздіктер ақиқат:}\\
&f^{\prime\prime}(x)\geq0\text{ (Қандай да бір I деген интервалда)}\Rightarrow \frac{\omega_1 f(x_1)+\omega_2 f(x_2)+\ldots+\omega_nf(x_n)}{\omega_1+\omega_2+\ldots+\omega_n}\geq f(\frac{\omega_1x_1+\omega_2x_2+\ldots+\omega_nx_n}{\omega_1+\omega_2+\ldots+\omega_n}); x_1,\ldots, x_n\in I, \omega_1,\ldots,\omega_n\geq 0 \\
&f^{\prime\prime}(x)\leq0\text{ (Қандай да бір I деген интервалда)}\Rightarrow \frac{\omega_1 f(x_1)+\omega_2 f(x_2)+\ldots+\omega_nf(x_n)}{\omega_1+\omega_2+\ldots+\omega_n}\leq f(\frac{\omega_1x_1+\omega_2x_2+\ldots+\omega_nx_n}{\omega_1+\omega_2+\ldots+\omega_n}); x_1,\ldots, x_n\in I, \omega_1,\ldots,\omega_n\geq 0 \\
